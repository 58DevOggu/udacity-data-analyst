...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            ...
            class_dict = {}
            for field in fields:
                if fields[field] in ["label", "uri", "description", "name", "synonym"]:
                    newDict[fields[field]]=line[field].strip()
                else:
                    class_dict[fields[field]] = line[field].strip()
                    if class_dict[fields[field]] == "NULL":
                        class_dict[fields[field]] = None
            newDict['classification'] = class_dict           
            for newKey in newDict.keys():
                if newDict[newKey]=="NULL" and newKey!="name":
                    newDict[newKey]=None
                if newKey=='label':
                    newDict["label"]=newDict["label"].split("(")[0].strip()
                if newKey=='name':
                    #print "Name before Lable" +newDict["name"]
                    #print "IsAplha Num" +str(newDict["name"].isalnum())
                    if newDict["name"] =="NULL" or (not newDict["name"].isalnum()):
                        newDict["name"]=newDict["label"].strip()
                        #print "Name is " +newDict["name"]
                if newKey=="synonym" and newDict["synonym"] is not None:
                   
                    temp_array = parse_array(newDict[newKey])
                    newDict["synonym"] = clean_array(temp_array)
                    #print "Synonym Values" +str(newDict[newKey])
            data.append(newDict)   
