...
def fix_turnstile_data(filenames):
    ...
    for name in filenames:
        ...
        for row in reader:
                ...
                new_line=[row[i:i+5] for i in range(3, len(row), 5)]
                for i in new_line:
                    writer.writerow(row[:3] + i)
...
def time_to_hour(time):
    ...
    hour = tim.strptime(time,"%H:%M:%S")[3]
    return hour
...
#    PRTOBLEM SET2 - 2#    PRTOBLEM SET3 - 3
# In[ ]:
